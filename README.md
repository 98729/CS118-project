...
## Project Report
### Team members
605725838 Yijun Cheng
205726383 Qingyuan Hu
805728143 Yuedong Cui

### High level design
Our project idea starts with designing a server that can keep accepting clients both in the public network & private network. Upon using a thread for each client, our server can choose what to do based on the conditions:
(1) LAN to LAN direct packet forwarding if both src IP and dest IP are in the same subnet
(2) Look up in the static NAPT Table and if entries are found, do forwarding
(3) Dynamically map IP and port number and look up each time in the dynamic NAPT table and do forwarding

Our server also calculates checksum and TTL each time when a new packet is received to make sure that invalid/expired packet can be dropped.

### Challenges
We've encountered several problems while implementing our server.

(1) To create a thread for each client and record the socket descriptor
We've kept being prompted with connection errors at the beginning because we always mapped the client with a wrong socket descriptor. In the end, we manually mapped all IPs and descriptors and do all the forwarding operation in thread functions.

(2) We spent quite a lot of time calculating checksum
Turned out that to calculate checksum, we will need to add not only the header of tcp/udp but also pseudo header (IP, protocol, length)
